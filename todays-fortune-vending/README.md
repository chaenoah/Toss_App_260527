...
- ✅ Phase 1 — 온보딩~결과~공유 핵심 플로우
- ✅ Phase 2 — streak 복구권 모달 + 뱃지 달성 연출
- ✅ Phase 3 — 주말 특집 "이번 주 재물운 총평" 화면(주간 그래프·평균·베스트/워스트)
- ✅ Phase 4 — 공유 리워드(복주머니, 일 1회 캡) + 복주머니로 상세 리포트 해제 + 광고 이벤트 파싱 정교화 + 트래킹 확장(screen_view 등)

### 남은 것 (출시 준비)

- `granite.config.ts` 의 `brand.icon` 설정
- 콘솔 광고 그룹 생성 → `src/config/ads.ts` 에 ID 입력 + `enabled: true`
- 개인정보 처리방침(생년월일 수집 고지)
- 실기기 광고·수익 검증 → 심사 요청
- (선택) 돈 궁합 · 시장 정보
