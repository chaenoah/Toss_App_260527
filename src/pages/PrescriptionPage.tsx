...
export function PrescriptionPage({ record, onBack, onRetry }: Props) {
  const { show } = useToast();
  const ticketRef = useRef<HTMLDivElement>(null);
  const [sharing, setSharing] = useState(false);

  const { emotion, intensity, prescription: rx, memo } = record;
  const intensityLabel = ['', '아주 약한', '약한', '보통의', '강한', '아주 강한'][intensity];
  ...
  async function handleShare() {
    ...
    const text = `[감정 자판기 처방전]\n${emotion.emoji} ${intensityLabel} ${emotion.label}\n\n🎨 ${rx.color.hex} — ${rx.color.description}\n🎵 ${rx.song.title} — ${rx.song.artist}\n💬 "${rx.quote.text}" — ${rx.quote.author}\n🎯 ${rx.mission}\n💙 ${rx.comfort}`;
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-6">
        ...
        <div ref={ticketRef} className="ticket-enter bg-[#F3F0EA] rounded-3xl overflow-hidden shadow-xl">
          ...
          <div
            ...
            style={{ background: `linear-gradient(135deg, ${emotion.gradientFrom}, ${emotion.gradientTo})` }}
          >
            <div className="text-4xl mb-1">{emotion.emoji}</div>
            <div className="font-bold text-gray-900 text-lg">{intensityLabel} {emotion.label}</div>
            <div className="text-xs text-gray-600 mt-0.5">{record.date}</div>
            {memo && (
          ...
          <div className="px-4 space-y-4 pb-4">
            <PrescriptionSection icon="🎨" label="오늘의 색">
              <div className="flex items-center gap-3">
                ...
                />
                <div>
                  <div className="font-bold text-gray-900 text-sm">{rx.color.hex}</div>
            ...
            <PrescriptionSection icon="🎵" label="오늘의 곡">
            ...
            <PrescriptionSection icon="💬" label="오늘의 한 줄">
            ...
            <PrescriptionSection icon="🎯" label="오늘의 미션">
              <div className="text-sm text-gray-800 leading-relaxed">{rx.mission}</div>
            </PrescriptionSection>

            <Divider />

            <PrescriptionSection icon="💙" label="오늘의 위로">
...
function PrescriptionSection({ icon, label, children }: { icon: string; label: string; children: React.ReactNode }) {
  return (
    <div>
