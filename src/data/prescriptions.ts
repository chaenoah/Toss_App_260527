import type { Emotion, Prescription } from '../types';
...
const POOLS: Record<string, PrescriptionPool> = {
  joy: {
    colors: [
      { hex: '#FFE066', description: '햇살처럼 반짝이는 황금빛이에요' },
      { hex: '#FFC8A2', description: '복숭아처럼 달콤하고 환한 색이에요' },
      { hex: '#FFFDE7', description: '따뜻한 오후 햇살을 담은 크림색이에요' },
    ],
    songs: [
    ...
    quotes: [
      ...
      { text: '행복한 순간을 놓치지 마세요. 지금 이 순간이 나중에 그리워질 추억이 됩니다.', author: '익명' },
  ...
  excited: {
    colors: [
      { hex: '#FFB6C1', description: '설레는 마음처럼 살짝 빨개진 분홍이에요' },
      { hex: '#FF8FAB', description: '두근거리는 심장처럼 생기 넘치는 색이에요' },
      { hex: '#FFDDE6', description: '첫사랑처럼 수줍고 포근한 분홍이에요' },
    ],
    songs: [
    ...
    quotes: [
      { text: '설렘이란, 아직 오지 않은 좋은 일이 오는 소리다.', author: '익명' },
  ...
  calm: {
    colors: [
      { hex: '#B2DFDB', description: '잔잔한 호수처럼 맑고 고요한 민트예요' },
      { hex: '#E0F7FA', description: '구름 한 점 없는 하늘색이에요' },
      { hex: '#80DEEA', description: '선선한 바람 같은 아쿠아 블루예요' },
    ],
    songs: [
  ...
  proud: {
    colors: [
      { hex: '#A5D6A7', description: '성장하는 새싹처럼 신선한 초록이에요' },
      { hex: '#FFD700', description: '금메달처럼 빛나는 황금빛이에요' },
      { hex: '#81C784', description: '잘 자란 나무처럼 든든한 초록이에요' },
    ],
    songs: [
    ...
    quotes: [
      { text: '자신을 자랑스러워할 줄 아는 것도 용기다.', author: '익명' },
      { text: '작은 진전도 진전이다.', author: '익명' },
      { text: '오늘의 나는 어제의 나보다 조금 더 성장했다.', author: '익명' },
    ],
    missions: [
  ...
  grateful: {
    colors: [
      { hex: '#FFF9C4', description: '따스한 감사의 마음을 담은 연노랑이에요' },
      { hex: '#FFECB3', description: '꿀처럼 달콤하고 따뜻한 앰버예요' },
      { hex: '#FFF3E0', description: '아늑한 오후 햇살 같은 피치 크림이에요' },
    ],
    songs: [
    ...
    quotes: [
      { text: '감사하는 사람은 언제나 풍요롭다.', author: '익명' },
      { text: '오늘 하루에 감사할 것이 하나 이상이라면, 그것으로 충분하다.', author: '익명' },
  ...
  love: {
    colors: [
      { hex: '#FF8A80', description: '사랑에 빠진 마음처럼 붉고 따뜻한 색이에요' },
      { hex: '#FF80AB', description: '사랑스러운 핑크, 지금 당신의 색이에요' },
      { hex: '#FCE4EC', description: '포근하게 감싸는 연한 장밋빛이에요' },
    ],
    songs: [
    ...
    quotes: [
      ...
      { text: '사랑하는 사람 옆에 있으면, 세상이 조금 더 따뜻해진다.', author: '익명' },
    ],
    missions: [
  ...
  sad: {
    colors: [
      { hex: '#90A4AE', description: '조용히 내리는 비처럼 차분한 슬레이트 블루예요' },
      { hex: '#B0BEC5', description: '흐린 날 하늘처럼 은은하게 가라앉은 색이에요' },
      { hex: '#78909C', description: '깊은 물속처럼 고요하고 깊은 색이에요' },
    ],
    songs: [
    ...
    quotes: [
      { text: '슬픔 뒤에는 반드시 기쁨이 온다. 그래서 슬픔도 소중하다.', author: '익명' },
      { text: '눈물은 마음이 쉬는 방법이다.', author: '익명' },
      { text: '울어도 괜찮아. 비가 와야 무지개가 뜨니까.', author: '익명' },
    ],
    missions: [
      '이불 속에 5분만 파묻혀 있기',
  ...
  lonely: {
    colors: [
      { hex: '#CE93D8', description: '아무도 없는 밤의 보랏빛 하늘이에요' },
      { hex: '#9C27B0', description: '혼자만의 깊고 깊은 자색이에요' },
      { hex: '#E1BEE7', description: '홀로 피어난 라벤더처럼 은은한 색이에요' },
    ],
    songs: [
    ...
    quotes: [
      ...
      { text: '외로움은 당신이 자기 자신과 더 가까워지는 시간이다.', author: '익명' },
  ...
  depressed: {
    colors: [
      { hex: '#546E7A', description: '흐리고 어두운 날의 슬레이트 그레이예요' },
      { hex: '#607D8B', description: '무거운 마음처럼 차분하고 깊은 색이에요' },
      { hex: '#455A64', description: '깊은 밤 바다처럼 어둡고 고요한 색이에요' },
    ],
    songs: [
    ...
    quotes: [
      ...
      { text: '지금 힘든 것은 아직 살아있다는 의미다.', author: '익명' },
      { text: '폭풍은 영원히 계속되지 않는다.', author: '익명' },
    ],
    missions: [
  ...
  angry: {
    colors: [
      { hex: '#FF7043', description: '활활 타오르는 분노처럼 강렬한 오렌지 레드예요' },
      { hex: '#BF360C', description: '뜨거운 마음이 쏟아져 나오는 딥 레드예요' },
      { hex: '#FFCCBC', description: '분노가 잠잠해질 때의 부드러운 살구색이에요' },
    ],
    songs: [
  ...
  annoyed: {
    colors: [
      { hex: '#FFB74D', description: '은근히 끓어오르는 주황빛 짜증이에요' },
      { hex: '#FF8F00', description: '신호등처럼 눈에 띄는 앰버 오렌지예요' },
      { hex: '#FFF3E0', description: '짜증이 조금 가라앉을 때의 연한 오렌지예요' },
    ],
    songs: [
    ...
    quotes: [
      { text: '짜증은 내가 통제할 수 없는 것에 대한 반응이다. 내가 통제할 수 있는 것에 집중하자.', author: '익명' },
      { text: '기분이 나쁠 땐 잠시 멈추는 것이 최선이다.', author: '익명' },
      { text: '오늘의 짜증은 내일의 웃음거리가 된다.', author: '익명' },
    ],
    missions: [
  ...
  anxious: {
    colors: [
      { hex: '#CE93D8', description: '불안처럼 아른거리는 연보라예요' },
      { hex: '#9575CD', description: '진정되기를 기다리는 라벤더 퍼플이에요' },
      { hex: '#EDE7F6', description: '불안이 걷히고 나면 보이는 연한 라일락이에요' },
    ],
    songs: [
    ...
    quotes: [
      { text: '불안은 아직 일어나지 않은 일에 대한 두려움이다. 지금에 집중하라.', author: '익명' },
      { text: '걱정의 90%는 일어나지 않는다.', author: '미국 심리학회' },
  ...
  afraid: {
    colors: [
      { hex: '#A5D6A7', description: '두려움 속에서도 찾아볼 수 있는 연한 초록 희망이에요' },
      { hex: '#66BB6A', description: '두려움을 이겨낸 자리에 피어나는 초록이에요' },
      { hex: '#E8F5E9', description: '두려움이 차분히 가라앉을 때의 민트 크림이에요' },
    ],
    songs: [
    ...
    quotes: [
      ...
      { text: '두려움과 친해져라. 그것이 성장의 시작이다.', author: '익명' },
      { text: '두려움은 실패가 아니다. 포기가 실패다.', author: '익명' },
    ],
    missions: [
  ...
  empty: {
    colors: [
      { hex: '#ECEFF1', description: '아무것도 채워지지 않은 듯한 페일 블루그레이예요' },
      { hex: '#CFD8DC', description: '텅 빈 방의 고요한 회색이에요' },
      { hex: '#B0BEC5', description: '허무함처럼 희미하게 남아있는 실버 그레이예요' },
    ],
    songs: [
    ...
    quotes: [
      { text: '허무함은 새로운 것을 채울 준비가 된 상태다.', author: '익명' },
      { text: '비어있어야 채울 수 있다.', author: '노자' },
      { text: '지금 이 텅 빈 느낌도 지나간다.', author: '익명' },
    ],
    missions: [
  ...
  tired: {
    colors: [
      { hex: '#D7CCC8', description: '지친 몸처럼 부드럽고 따뜻한 그레이 베이지예요' },
      { hex: '#BCAAA4', description: '눈꺼풀처럼 무겁게 가라앉은 로즈 브라운이에요' },
      { hex: '#EFEBE9', description: '깊은 잠에 들기 직전의 크리미 베이지예요' },
    ],
    songs: [
      { title: 'Sleeping at Last - Saturn', artist: 'Sleeping at Last', reason: '지친 몸을 쉬게 해주는 음악이에요' },
      { title: '잠', artist: '멜로망스', reason: '포근하게 잠들고 싶을 때 딱이에요' },
      { title: 'Nuvole Bianche', artist: 'Ludovico Einaudi', reason: '피아노 선율이 지친 마음을 쉬게 해줘요' },
    ],
    quotes: [
      { text: '잘 쉬는 것도 능력이다.', author: '익명' },
      { text: '지쳤을 때 멈추는 건 포기가 아니라 전략이다.', author: '익명' },
      { text: '몸이 쉬고 싶다고 하면, 들어줘야 한다.', author: '익명' },
    ],
    missions: [
      '지금 당장 눈 감고 10분만 쉬기',
  ...
  blank: {
    colors: [
      { hex: '#E3F2FD', description: '아무 생각 없이 보는 하늘처럼 맑고 연한 블루예요' },
      { hex: '#BBDEFB', description: '멍하게 바라보는 창밖 같은 스카이 블루예요' },
      { hex: '#F8F9FA', description: '아무것도 아닌 것들로 가득한 흰 공간이에요' },
    ],
    songs: [
    ...
    quotes: [
      { text: '멍하게 있는 것도 뇌가 정리되는 시간이다.', author: '뇌과학 연구' },
      { text: '아무 생각 없이 있어도 괜찮다. 그냥 지금 있으면 된다.', author: '익명' },
      { text: '멍은 마음의 방전이 아니라 재충전이다.', author: '익명' },
    ],
    missions: [
      '창밖을 5분간 그냥 바라보기',
      '핸드폰 내려놓고 1분간 멍때리기',
...
export function generatePrescription(emotion: Emotion, intensity: number, memo: string, seed?: number): Prescription {
  const pool = POOLS[emotion.id] ?? POOLS['calm'];
